...
if ( !class_exists( 'ITMPlugin' ) ) {
    ...
    */
    class ITMPlugin{
        ...
        * Settings
        */
        private $data_saving, $override_alt, $override_title, $preserved_char, $string_case,$js_filter, $js_log;
        /**
        ...
        */
        private $strip_numbers,$use_post_title_as_default, $add_post_title_to_alt, $bar_separator, $add_post_title_to_title, $string_trimmer, $remove_from_alt, $remove_from_title;
        /**
        ...
        */
        ...
        public function Initilized(){
            ...
            */
            ...
            $this->js_filter=get_option("js_filter");
            $this->js_log=get_option("js_log");

            $this->preserved_char=get_option("preserved_char");
            $this->string_case=get_option("string_case");
            $this->bar_separator=get_option("bar_separator");
            ...
            */
            ...
            if(boolval($this->js_filter)){
                add_action( 'wp_enqueue_scripts', array($this,"FrontScripts") );
                add_action( 'wp_ajax_itm_ajaxrequest', array($this,"AjaxRequest") );
                add_action( 'wp_ajax_nopriv_itm_ajaxrequest', array($this,"AjaxRequest") );
            }
            ...
            */
            register_activation_hook(  plugin_basename( ITM_RELPATH . ITM_PLUGIN .'.php' ) , array( $this , 'Activate' ) );
        }
        public function Uninstall() {
            ...
            */
            register_uninstall_hook(   plugin_basename( ITM_RELPATH . ITM_PLUGIN .'.php' ) , array( $this , 'Erase' ) );
        }
        ...
        public static function Activate() {
            ...
            */
            ...
            if ( get_option( 'js_filter' ) === false ){ add_option( 'js_filter', 1, $this->deprecated, $this->autoload );}
            if ( get_option( 'js_log' ) === false ){ add_option( 'js_log', 0, $this->deprecated, $this->autoload );}

            if ( get_option( 'preserved_char' ) === false ){ add_option( 'preserved_char', 0, $this->deprecated, $this->autoload );}
            if ( get_option( 'string_case' ) === false ){ add_option( 'string_case', 3, $this->deprecated, $this->autoload );}

            if ( get_option( 'bar_separator' ) === false ){ add_option( 'bar_separator', 0, $this->deprecated, $this->autoload );}
        ...
        public static function Erase(){
            ...
            */
            ...
            delete_option( 'js_filter' );
            delete_option( 'js_log' );

            delete_option( 'preserved_char' );
            delete_option( 'string_case' );

            delete_option( 'bar_separator' );
        ...
        public function FrontScripts($hook_suffix) {
            ...
            */
            wp_register_script('ITM_ajaxrequest', ITM_ABSPATH.'admin/js/itm-ajaxrequest.js', array('terebra_achates', 'wp-util', 'json2'), 1.0, true);
            wp_localize_script( 'ITM_ajaxrequest', 'itm_ajaxrequest', $localize);
            wp_enqueue_script('ITM_ajaxrequest');
        }
        ...
        public function AjaxRequest(){
            ...
            if ( ! empty( $_POST ) ) {
                if(isset($_POST['filesrc'])){
                    $image_id = attachment_url_to_postid($_POST['filesrc']);
                    if($image_id){
                        $title = (!empty(get_post( $image_id )->post_title)) ? get_post( $image_id )->post_title : get_post( $image_id )->post_name;
                        $title = $this->Title($title, $_POST['filename']);

                        //Use the media attachment alt and fallback to image formatted name
                        $alt = get_post_meta( $image_id)['_wp_attachment_image_alt'][0];
                        $alt = (!empty($alt)) ? $alt : $title;
                    }else{
                        $title = $this->Title($_POST['filename'], $_POST['post_title']);
                        $alt = $title;
                    }
                    exit(wp_send_json(array(
                      'alt'    => $alt,
                      'title'  => $title
                  )));
                }
        ...
        public function Title( $image_title, $post_title = '' ) {
            ...
            */
            ...
            if(intval($this->preserved_char)!=1) {
            ...
            switch(intval($this->string_case)){
                case 1:
        ...
        public function Sanitizer($string, $is_file = false) {
            /**
            * This functions Transform the title into a valid and more decent Title
            * It remove unwanted characters
            */
            ...
            //Transform if preserved_char is set to false
            if(intval($this->preserved_char)!=1 || boolval($is_file)) {
        ...
        public function StandardizedFileName($image_filename, $post_title='') {
            ...
            */
            ...
            $image_name = $this->Sanitizer( $image_name, true );
            $blog_title = $this->Sanitizer( get_bloginfo('name'), true);
            $post_title = $this->Sanitizer( $post_title, true);
        ...
        public function Replace($image_row) {
            ...
            $image_formatted_name = $this->Title( $image_own_title, $image_filename['filename'] );

            //Get image own attachment alt
            $image_own_alt = get_post_meta( $image_id )['_wp_attachment_image_alt'][0];
            $image_own_alt = (!empty($image_own_alt)) ? $image_own_alt : $image_formatted_name;

            $new_image_alt = $this->Append($image_own_alt, $post_title, "alt");
            ...
            */
            ...
            if($count>0) {
                ...
                foreach($all_images[0] as $single_image) {
                    ...
                    /*
                    result for $image_src:
                    array(
                        [0] => array( [0] => src="filename.ext" )
                        [1] => array( [0] => filename.ext )
                    )
                    */

                    if(preg_match('/class="([^"]*wp-image-\d*)"/ims', stripslashes($single_image), $all_class)) {
                        $get_id = preg_match('/wp-image-\d+/i', $all_class[0], $filtered_class);
                        $image_id = trim(preg_replace('/wp-image-/i', '', $filtered_class[0]));
                    }
                    // else get the image id using the attachment_url_to_postid
                    else{
                        $image_id = attachment_url_to_postid($image_src[1][0]);
                    }
                    // for($i = 0; $i < count($image_src[1]); $i++) {
                        $image_filename = pathinfo($image_src[1][0]); //pathinfo($image_src[1][$i]);
                        //Get image own post_title or post_name
                        $image_own_title = (!empty(get_post( $image_id )->post_title)) ? get_post( $image_id )->post_title : get_post( $image_id )->post_name;
                        //Use image filename as fallback in no own image post_title or postname
                        $image_own_title = (!empty($image_own_title)) ? $image_own_title : $image_filename['filename'];
                        $image_formatted_name = $this->Title($image_own_title, $post_title);//$image_filename['filename']
                        ...
                        */
                        ...
                        if($with_alt == 0) {
                            ...
                            if(boolval($this->override_alt) && strtolower( trim($old_alt[1][0])) != strtolower( trim($new_image_alt))){
                        ...
                        */
                        ...
                        if($with_title == 0) {
                            ...
                            if(boolval($this->override_title) && strtolower( trim($old_title[1][0])) != strtolower( trim($new_image_title)) ){
